...
#Region "Métodos de la pagina"
    ...
    Private Sub imgbtnAgregar_Click(sender As Object, e As System.Web.UI.ImageClickEventArgs) Handles imgbtnAgregar.Click
        ...
        Response.Redirect("maRecibos.aspx")
    End Sub


    Private Sub grdListaRecibos_PageIndexChanging(sender As Object, e As System.Web.UI.WebControls.GridViewPageEventArgs) Handles grdListaRecibos.PageIndexChanging
        grdListaRecibos.PageIndex = e.NewPageIndex
        grdListaRecibos.DataSource = localData
        grdListaRecibos.DataBind()
        modificarCampoFechaGridView()
    End Sub

    Private Sub grdListaRecibos_RowCommand(sender As Object, e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles grdListaRecibos.RowCommand
        Dim vFilaActual As Integer = CInt(e.CommandArgument)
        If e.CommandName = "Eliminar" Then
            Session("vTransaccion") = "E"
            Session("vIdRecibo") = grdListaRecibos.Rows(vFilaActual).Cells(2).Text.Trim
            Response.Redirect("maRecibos.aspx")
        End If
    ...
    Protected Sub cargarGridView()
        ...
        Try
            ...
            vDataSet = vGestor.TbReciboConsultarRecibos()
            grdListaRecibos.DataSource = vDataSet
            grdListaRecibos.DataBind()
    ...
    Protected Sub cargarGridViewFiltros()
        ...
        Try
            ...
            If txtIdRecibo.Text = "" Then
                vRecibo.IdRecibo = 0
            ElseIf IsNumeric(txtIdRecibo.Text) Then
                vRecibo.IdRecibo = txtIdRecibo.Text
            End If

            vDataSet = vGestor.TbReciboConsultarReciboID(vRecibo)
            grdListaRecibos.DataSource = vDataSet
            grdListaRecibos.DataBind()
    ...
    Protected Sub modificarCampoFechaGridView()
        For i As Integer = 0 To grdListaRecibos.Rows.Count - 1
            grdListaRecibos.Rows(i).Cells(3).Text = grdListaRecibos.Rows(i).Cells(3).Text.Split(" ")(0)   'Se le quita la hora al campo fecha de emisión para que quede de tipo: 08/07/2014
        Next
    End Sub
